import { reportSchema, validateReportShape } from "./schema.js";
import { buildInputPrompt, buildInstructions, buildWindow } from "./prompt.js";
...
export function buildRequestBody(options, filterStyle = "nested") {
  const window = buildWindow(options.days);
  const webSearchTool =
    filterStyle === "nested" ? buildWebSearchTool(options.domains) : buildFlatWebSearchTool(options.domains);

  return {
    preset: options.preset,
    max_steps: options.maxSteps,
    input: buildInputPrompt({
      vendors: options.vendors,
      days: options.days,
      maxItemsPerVendor: options.maxItemsPerVendor,
      window
    }),
    instructions: buildInstructions(),
    tools: [webSearchTool, { type: "fetch_url" }],
    response_format: {
      type: "json_schema",
      json_schema: {
        name: "release_monitor_report",
        schema: reportSchema
      }
    }
  };
...
async function callAgentApi(apiKey, body) {
...
function shouldRetryWithFlatFilters(error) {
...
export async function runReleaseMonitor(options) {
  ...
  let requestBody = buildRequestBody(options, "nested");
  let response;
  ...
  try {
    ...
    requestBody = buildRequestBody(options, "flat");
    response = await callAgentApi(apiKey, requestBody);
  }
  ...
  const parsed = validateReportShape(extractJson(outputText));
